  ...
  asyncHandler(async (req, res) => {
    try {
      ...
      if (req.currentUser.id === course.userId) {
        ...
        //location header removed according to feedback ---> location(`/${req.params.id}`)
        res.status(403).json({ error: "You cannot update the course." });
      }
    } catch (error) {
      console.log("ERROR: ", error);
      if (
  ...
  asyncHandler(async (req, res) => {
    try {
      ...
      if (req.currentUser.id === course.userId) {
        ...
        res.status(403).json({ error: "Deletion cannot be done" });
      }
    } catch (error) {
      console.log("ERROR: ", error);
      if (
